...
public class ColdWallet {
    ...
    public static Credentials loadSingleNodeColdwallet() throws CipherException, IOException {
        ObjectMapper objectMapper = new ObjectMapper();
        String json = AccountsStr.cre_single_account;
        WalletFile walletFile = objectMapper.readValue(json, WalletFile.class);
        Credentials coldwallet = Credentials.create(Wallet.decrypt(AccountsStr.cre_pwd, walletFile));
        return coldwallet;
    }
    ...
    public static String sendRawTxs(int peerID,String toAddress, float value, Credentials coldwallet,
        ...
        //设置需要的矿工费
        BigInteger GAS_PRICE = BigInteger.valueOf(15000);
        if (defaultGasPrice) {
            GAS_PRICE = BigInteger.valueOf(5000000000L);
        }
        BigInteger GAS_LIMIT = new BigInteger("4300000");
